...
int HI_COMMON_OpenFramebufferDevice(int width, int height, display_format display_fmt)
{
    ...
    if (-1 == fd)
    {
        MALI_FPRINT("open /dev/fb0 failed\n");
        return -1;
    }

    if (ioctl(fd, FBIOPUT_ALPHA_HIFB, &stAlpha) < 0)
    {
        MALI_FPRINT("Unable to set alpha!\n");
        return -1;
    }

    if ((err = ioctl(fd, FBIOGET_VSCREENINFO, &vinfo)) < 0)
    {
        MALI_FPRINT("Unable to FBIOGET_VSCREENINFO!, err=0x%x\n", err);
        return -1;
    }
    ...
    switch(display_fmt)
    {
        ...
        default:
            MALI_FPRINT("display format invalid\n");
            return -1;
            break;
    }
    ...
    {
        MALI_FPRINT("Unable to set double buffer mode!, err=0x%x\n", err);
        vinfo.yres_virtual = vinfo.yres;
        if ((err = ioctl(fd, FBIOPUT_VSCREENINFO, &vinfo)) < 0)
        {
            MALI_FPRINT("Unable to FBIOPUT_VSCREENINFO!, err=0x%x\n", err);
            return -1;
        }
...
void * HI_COMMON_LoadFile(const char *f_path, unsigned long skip, unsigned long f_size, const char *f_type)
{
    ...
    if (fptr == NULL)
    {
        MALI_FPRINT("Can not open file: %s\n", f_path);
        return NULL;
    }
    ...
    /* error handle */
    MALI_FPRINT("error while read resource file!\n");
...
void HI_COMMON_PostMemoryToFramebuffer(void *virtual_addr, int memsize)
{
    ...
    {
        MALI_FPRINT( "Couldn't get console hardware info\n");
        goto CLOSEFD;
    }
    ...
    {
        MALI_FPRINT( "Unable to memory map the video hardware\n");
        mapped_mem = NULL;
        goto CLOSEFD;
    }
    ...
    {
        MALI_FPRINT( "Couldn't get vscreeninfo\n");
        goto UNMAP;
    }
...
HI_MMZ_BUF_S * HI_COMMON_CreateMMZMemory(unsigned int size)
{
    ...
    HI_MPI_SYS_MmzAlloc(&pBuf->phyaddr, (HI_VOID**)&pBuf->viraddr, NULL, NULL, size);
...
{
    HI_MPI_SYS_MmzFree(pBuf->phyaddr, pBuf->viraddr);
}
