...
static HI_S32 CONFACCESS_Init(const HI_CHAR* pszCfgName, const HI_CHAR* pszCfgPath, HI_U32* pu32ModuleNum)
{
    ...
    const HI_CHAR* pazModuleDefPath = iniparser_getstring(pstCfgPos->stModule.pstIniDir,
                "module:path", CONFACCESS_DEFAULT_MODULE_CFGPATH);
    *pu32ModuleNum = s32ModuleNum;
    ...
    for (s32Idx = 0; s32Idx < s32ModuleNum; ++s32Idx)
    {
        ...
        /* Module Path */
        snprintf(szModuleNodeKey, HI_CONFACCESS_KEY_MAX_LEN, "%s:path", pstModuleInfo->szName);
        pazString = iniparser_getstring(pstCfgPos->stModule.pstIniDir, szModuleNodeKey, NULL);
        if (NULL == pazString || NULL == strchr(pazString, CONFACCESS_PATH_SEPARATOR))
        {
            snprintf(pstModuleInfo->szPath, HI_CONFACCESS_PATH_MAX_LEN, "%s", pazModuleDefPath);
            MLOGW("Invalid module path, use default path[%s]\n", pazModuleDefPath);
        }
        else
        {
            snprintf(pstModuleInfo->szPath, HI_CONFACCESS_PATH_MAX_LEN, "%s", pazString);
        }
