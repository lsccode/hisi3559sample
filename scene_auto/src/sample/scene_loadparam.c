...
HI_S32 HI_SCENE_CreateParam(HI_CHAR * pdirname, HI_SCENE_PARAM_S* pstSceneParam, HI_SCENE_VIDEO_MODE_S* pstVideoMode)
{
	...
	HI_CHAR * inipath;	
    HI_CHAR acInipath0[SCENETOOL_MAX_FILESIZE]= {0};
	HI_CHAR acInipath1[SCENETOOL_MAX_FILESIZE]= {0};
	inipath = "../../../src/sensor/";
	...
	snprintf(acInipath0, SCENETOOL_MAX_FILESIZE, "%s%s", inipath, pdirname);
	snprintf(acInipath1, SCENETOOL_MAX_FILESIZE, "%s%s", acInipath0, "/param/config_cfgaccess_hd.ini");
    printf("The iniPath is %s.\n",acInipath1);

    s32Ret = HI_CONFACCESS_Init(SCENE_INIPARAM, acInipath1, &u32ModuleNum);
    if(HI_SUCCESS != s32Ret)
    {
