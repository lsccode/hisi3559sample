...
static HI_S32 SCENE_SetPipeStaticParam(HI_VOID)
{
    ...
    if (HI_TRUE == s_stModuleState.bStaticNr)
    {
        for (i = 0; i < HI_SCENE_PIPE_MAX_NUM; i++)
        {
            if (HI_TRUE != s_stSceneMode.astPipeAttr[i].bEnable)
            ...
            s32Ret = HI_SCENE_SetStaticNr_AutoGenerate(s_stSceneMode.astPipeAttr[i].VcapPipeHdl, s_stSceneMode.astPipeAttr[i].u8PipeParamIndex);
            HI_SCENECOMM_CHECK_RETURN(s32Ret, HI_SCENE_EINTER);
        }
    }
    ...
    if (HI_TRUE == s_stModuleState.bStaticThreeDNR)
    {
        for (i = 0; i < HI_SCENE_PIPE_MAX_NUM; i++)
        {
            if (HI_TRUE != s_stSceneMode.astPipeAttr[i].bEnable)
            ...
            s32Ret = HI_SCENE_SetStaticThreeDNR_AutoGenerate(s_stSceneMode.astPipeAttr[i].VcapPipeHdl, s_stSceneMode.astPipeAttr[i].u8PipeParamIndex);
            HI_SCENECOMM_CHECK_RETURN(s32Ret, HI_SCENE_EINTER);
        }
    }
...
static HI_S32 SCENE_SetPipeDynamicParam(HI_VOID)
{
    ...
    if (HI_TRUE == s_stModuleState.bDyanamicFalseColor)
    {
    ...
    if (HI_TRUE == s_stModuleState.bDyanamicDemosaic)
    {
        SCENE_SET_DYNAMIC_BYPARAM_CALLBACK_FN_PTR pFuncDemosaic = HI_SCENE_SetDynamicDemosaic_AutoGenerate;
        s32Ret = SCENE_SetDynamicParam_byParam(pFuncDemosaic, SCENE_THREAD_TYPE_NORMAL, SCENE_DYNAMIC_CHANGE_TYPE_EXPOSURE);
        HI_SCENECOMM_CHECK_RETURN(s32Ret, HI_SCENE_EINTER);
    }
    ...
    if (HI_TRUE == s_stModuleState.bDyanamicDIS)
    {
        for (i = 0; i < s_stSceneState.u32MainPipeNum; i++)
        {
            for (j = 0; j < s_stSceneState.astMainPipe[i].u32SubPipeNum; j++)
...
HI_VOID* SCENE_LuminanceAutoThread(HI_VOID* pVoid)
{
    ...
    while (HI_TRUE == s_stSceneState.stThreadLuminance.bThreadFlag)
    {
        ...
        if (HI_TRUE == s_stModuleState.bDyanamicGamma)
        {
           for (i = 0; i < s_stSceneState.u32MainPipeNum; i++)
           {
               for (j = 0; j < s_stSceneState.astMainPipe[i].u32SubPipeNum; j++)
...
HI_VOID* SCENE_NotLinearAutoThread(HI_VOID* pVoid)
{
    ...
    while (HI_TRUE == s_stSceneState.stThreadNotLinear.bThreadFlag)
    {
        ...
        {
            VI_PIPE ViPipe = s_stSceneState.astMainPipe[i].MainPipeHdl;
            if (HI_SCENE_PIPE_MODE_HDR == s_stSceneMode.enPipeMode && HI_TRUE == s_stModuleState.bDynamicHDRDRC)
            {
                s32Ret = HI_SCENE_SetDynamicHDRDRC_AutoGenerate(ViPipe, s_stSceneMode.astPipeAttr[ViPipe].VpssHdl, s_stSceneMode.astPipeAttr[ViPipe].VPortHdl,
                                                              s_stSceneState.astMainPipe[i].u32HDRBrightRatio, s_stSceneMode.astPipeAttr[ViPipe].u8PipeParamIndex);
                HI_SCENECOMM_CHECK(s32Ret, HI_SCENE_EINTER);
            }
        }
        ...
        {
            VI_PIPE ViPipe = s_stSceneState.astMainPipe[i].MainPipeHdl;
            if (HI_SCENE_PIPE_MODE_HDR == s_stSceneMode.enPipeMode && HI_TRUE == s_stModuleState.bDynamicHDROETF)
            {
                s32Ret = HI_SCENE_SetDynamicHDROTEF_AutoGenerate(ViPipe, s_stSceneMode.astPipeAttr[ViPipe].VpssHdl, s_stSceneMode.astPipeAttr[ViPipe].VPortHdl,
                                                               s_stSceneState.astMainPipe[i].u32HDRBrightRatio, s_stSceneMode.astPipeAttr[ViPipe].u8PipeParamIndex);
                HI_SCENECOMM_CHECK(s32Ret, HI_SCENE_EINTER);
            }
        }
        ...
        {
            VI_PIPE ViPipe = s_stSceneState.astMainPipe[i].MainPipeHdl;
            if (HI_SCENE_PIPE_MODE_HDR == s_stSceneMode.enPipeMode && HI_TRUE == s_stModuleState.bDynamicHDRTM)
            {
                s32Ret = HI_SCENE_SetDynamicHDRTM_AutoGenerate(ViPipe, s_stSceneMode.astPipeAttr[ViPipe].VpssHdl, s_stSceneMode.astPipeAttr[ViPipe].VPortHdl,
                                                               s_stSceneState.astMainPipe[i].u32HDRBrightRatio, s_stSceneMode.astPipeAttr[ViPipe].u8PipeParamIndex);
                HI_SCENECOMM_CHECK(s32Ret, HI_SCENE_EINTER);
            }
        }
