            ...
            HI_CHAR *pchFgFileName,HI_CHAR *pchBgFileName)
{
    ...
    pu8Tmp = (HI_U8*)pstGmm2->stFactor.au64VirAddr[0];
   ...
   memset((HI_VOID*)pstGmm2->stModel.u64VirAddr, 0, pstGmm2->stModel.u32Size);
...
static HI_VOID SAMPLE_IVE_ReduFactor(IVE_IMAGE_S *pstFactor)
{
    ...
    pu8Tmp = (HI_U8*)pstFactor->au64VirAddr[0];
                                                        ...
                                                        IVE_DST_IMAGE_S *pstDiffImg,IVE_DST_IMAGE_S *pstFactor)
{
    ...
    pu8Tmp = (HI_U8*)pstDiffImg->au64VirAddr[0];
    ...
    {
        pu8Tmp   = (HI_U8*)pstDiffImg->au64VirAddr[0];
        pu8Factor = (HI_U8*)pstFactor->au64VirAddr[0];
        for (i = 0; i < pstSrc->u32Height; i++)
        {
            for (j = 0; j < pstSrc->u32Width; j++)
            {
                ...
                IVE_DST_IMAGE_S *pstDiffImg,IVE_DST_IMAGE_S *pstFactor)
{
    ...
    for(k = 0; k < IVE_MAX_REGION_NUM; k++)
    {
        ...
        s32FgSum    = 0;
        s32GradSum  = 0;
        pu8FgCur    = ((HI_U8*)pstFgMask->au64VirAddr[0]) + u16Top * pstFgMask->au32Stride[0];
        pu8GradDiff = ((HI_U8*)pstDiffImg->au64VirAddr[0]) + u16Top * pstDiffImg->au32Stride[0];
        ...
        {
            pu8Factor = ((HI_U8*)pstFactor->au64VirAddr[0]) + 2 * u16Top * pstFactor->au32Stride[0];
            pu8FgCur = ((HI_U8*)pstFgMask->au64VirAddr[0]) + u16Top * pstFgMask->au32Stride[0];
            for (i = u16Top; i < u16Bottom; i++)
            {
                for (j = u16Left; j < u16Right; j++)
