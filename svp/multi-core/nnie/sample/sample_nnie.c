...
//    HI_CHAR *pcModelName = "./data/nnie_model/detection/inst_ssd_inst.wk";
    ...
    /*Ssd Load model*/
    SAMPLE_SVP_TRACE_INFO("Ssd Load model!\n");
    s32Ret = SAMPLE_COMM_SVP_NNIE_LoadModel(pcModelName,&s_stSsdCaffeModel);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
                               "Error,SAMPLE_COMM_SVP_NNIE_LoadModel failed!\n");
    ...
    /*Ssd parameters are set in SAMPLE_SVP_NNIE_Ssd_SoftwareInit,
      ...
      SAMPLE_SVP_NNIE_Ssd_SoftwareInit function are correct*/
    SAMPLE_SVP_TRACE_INFO("Ssd parameter initialization!\n");
    s_stSsdCaffeNnieParam.pstModel = &s_stSsdCaffeModel.stModel;

    struct timespec stStartTimeParamInit;
    struct timespec stEndTimeParamInit;
    clock_gettime(CLOCK_MONOTONIC, &stStartTimeParamInit);
    s32Ret = SAMPLE_SVP_NNIE_Ssd_ParamInit(&stNnieCfg,&s_stSsdCaffeNnieParam,&s_stSsdCaffeSoftwareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
                               "Error,SAMPLE_SVP_NNIE_Ssd_ParamInit failed!\n");
    ...
    clock_gettime(CLOCK_MONOTONIC, &stStartTimeParamInit);
     ...
     printstNnieCfg(&stNnieCfg);
     printstNnieParam(&s_stSsdCaffeNnieParam,s_stSsdCaffeModel.stModel.u32NetSegNum);
    /*Fill src data*/
...
//    fprintf(stderr,"Ssd start!\n");
    ...
    while(1)
    {
        fprintf(stderr,"\n################ Hi3559 ARFCV100 Ssd Caffe profiling start procesing ##############\n");
        fprintf(stderr,"Image resolution : 300*300\n");
        //clock_gettime(CLOCK_MONOTONIC, &stStartTimeSpc);
        countIn = 0;
        while(countIn++ < M_INTER_LOOP_NUMBER)
        {
            ...
            clock_gettime(CLOCK_MONOTONIC, &stStartSpcTime);
#if 0
            s32Ret = SAMPLE_SVP_NNIE_FillSrcData(&stNnieCfg,&s_stSsdCaffeNnieParam,&stInputDataIdx);
            SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
                                       "Error,SAMPLE_SVP_NNIE_FillSrcData failed!\n");
            fprintf(stderr,"SSD0 Line = %d\n",__LINE__);
#else
            ...
            stProcSegIdx.u32SegIdx = 0;
            clock_gettime(CLOCK_MONOTONIC, &stStartSpcForward);
            s32Ret = SAMPLE_SVP_NNIE_Forward(&s_stSsdCaffeNnieParam,&stInputDataIdx,&stProcSegIdx,HI_TRUE);
            SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
                                       "Error,SAMPLE_SVP_NNIE_Forward failed!\n");
            //fprintf(stderr,"SSD0 Line = %d\n",__LINE__);
            /*software process*/
            ...
            HI_DOUBLE dbGetResDiff = SAMPLE_SVP_GetDiff(&stStartSpcGetRes,&stEndSpcTime);
            //fprintf(stderr,"SSD0 Line = %d\n",__LINE__);
            SAMPLE_SVP_PrintPerformance("Ssd Caffe profiling",dbTotalDiff,dbFillSrcDiff,dbFrowardDiff,dbGetResDiff);
            //fprintf(stderr,"SSD0 Line = %d\n",__LINE__);
        }
//        clock_gettime(CLOCK_MONOTONIC, &stEndTimeSpc);
//        HI_U64 diff = stEndTimeSpc.tv_sec*1000*1000 +
...
//        fprintf(stderr,"  CNN Performance  : %.2f fps\n",1000*1000*1.0*M_INTER_LOOP_NUMBER/diff);
        ...
        fprintf(stderr,"################ Hi3559 ARFCV100 Ssd Caffe profiling end procesing ################\n");
        //(void)SAMPLE_SVP_NNIE_Detection_PrintResult(&s_stSsdSoftwareParam0.stDstScore,
        //        &s_stSsdSoftwareParam0.stDstRoi, &s_stSsdSoftwareParam0.stClassRoiNum,f32PrintResultThresh);
    }

    (void)SAMPLE_SVP_NNIE_Detection_PrintResult(&s_stSsdCaffeSoftwareParam.stDstScore,
            &s_stSsdCaffeSoftwareParam.stDstRoi, &s_stSsdCaffeSoftwareParam.stClassRoiNum,f32PrintResultThresh);

SSD_FAIL_0:
    SAMPLE_SVP_NNIE_Ssd_Deinit(&s_stSsdCaffeNnieParam,&s_stSsdCaffeSoftwareParam,&s_stSsdCaffeModel);
    SAMPLE_COMM_SVP_CheckSysExit();
}
...
{
    SAMPLE_SVP_NNIE_Ssd_Deinit(&s_stSsdCaffeNnieParam,&s_stSsdCaffeSoftwareParam,&s_stSsdCaffeModel);
    memset(&s_stSsdCaffeNnieParam,0,sizeof(SAMPLE_SVP_NNIE_PARAM_S));
    memset(&s_stSsdCaffeSoftwareParam,0,sizeof(SAMPLE_SVP_NNIE_SSD_SOFTWARE_PARAM_S));
    memset(&s_stSsdCaffeModel,0,sizeof(SAMPLE_SVP_NNIE_MODEL_S));
    SAMPLE_COMM_SVP_CheckSysExit();
}
