...
*
*****************************************************************************/
static HI_S32 SVP_NNIE_Ssd_DetectionOutForward(char *name,HI_U32 u32ConcatNum,
...
static HI_S32 SVP_NNIE_Yolov1_Iou(HI_FLOAT *pf32Bbox, HI_U32 u32Idx1, HI_U32 u32Idx2)
{
    ...
    f32WidthDis = SAMPLE_SVP_NNIE_MAX(pf32Bbox[u32Idx1*SAMPLE_SVP_NNIE_COORDI_NUM] +
    ...
    f32HeightDis = SAMPLE_SVP_NNIE_MAX(pf32Bbox[u32Idx1*SAMPLE_SVP_NNIE_COORDI_NUM+1] +
    ...
    HI_U32 u32BboxNum,HI_U32 u32ConfThresh,HI_U32 u32NmsThresh,HI_U32* pu32TmpBuf)
{
    ...
    SAMPLE_SVP_NNIE_STACK_S* pstAssitBuf = (SAMPLE_SVP_NNIE_STACK_S*)((HI_U8*)pu32TmpBuf+
        u32BboxNum*sizeof(SAMPLE_SVP_NNIE_YOLOV1_SCORE_S) * sizeof(HI_U32));
    ...
    HI_S32 *ps32ClassRoiNum)
{
    ...
    //HI_U32 u32ParamSize = u32GridNum*u32Length*sizeof(HI_FLOAT);
...
*
*****************************************************************************/
HI_S32 SAMPLE_SVP_NNIE_Ssd_GetResult(char *name ,SAMPLE_SVP_NNIE_PARAM_S*pstNnieParam,
    SAMPLE_SVP_NNIE_SSD_SOFTWARE_PARAM_S* pstSoftwareParam)
{
...
//    fprintf(stderr,"%s Line = %d\n",name,__LINE__);
    ...
    //fprintf(stderr,"%s u32ConcatNum = %u,Line = %d\n",name,pstSoftwareParam->u32ConcatNum,__LINE__);
    //fprintf(stderr,"%s u32ConfThresh = %u,Line = %d\n",name,pstSoftwareParam->u32ConfThresh,__LINE__);
    //fprintf(stderr,"%s u32ClassNum = %u,Line = %d\n",name,pstSoftwareParam->u32ClassNum,__LINE__);
    //fprintf(stderr,"%s u32TopK = %u,Line = %d\n",name,pstSoftwareParam->u32TopK,__LINE__);
    //fprintf(stderr,"%s u32KeepTopK = %u,Line = %d\n",name,pstSoftwareParam->u32KeepTopK,__LINE__);
    //fprintf(stderr,"%s u32NmsThresh = %u,Line = %d\n",name,pstSoftwareParam->u32NmsThresh,__LINE__);
    //fprintf(stderr,"%s au32DetectInputChn = %u,Line = %d\n",name,pstSoftwareParam->au32DetectInputChn,__LINE__);

    (void)SVP_NNIE_Ssd_DetectionOutForward(name,pstSoftwareParam->u32ConcatNum,
