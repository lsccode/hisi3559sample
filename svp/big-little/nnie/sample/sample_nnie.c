...
static SVP_MEM_INFO_S s_stRfcnRectBuf = {0};
...
static HI_VOID* SAMPLE_SVP_NNIE_Rfcn_ViToVo(HI_VOID* pArgs)
{
    ...
    SAMPLE_NNIE_DRAW_RECT_MSG_S stDrawRectMsg;
    ...
    {
        s32Ret = SAMPLE_VPSS_GetChnFrame(s32VpssGrp, as32VpssChn[1], &stExtFrmInfo, s32MilliSec);
        if(HI_SUCCESS != s32Ret)
        {
            SAMPLE_PRT("Error(%#x),SAMPLE_VPSS_GetChnFrame failed, VPSS_GRP(%d), VPSS_CHN(%d)!\n",
                s32Ret,s32VpssGrp, as32VpssChn[1]);
            continue;
        }

        s32Ret = SAMPLE_VPSS_GetChnFrame(s32VpssGrp, as32VpssChn[0], &stBaseFrmInfo, s32MilliSec);
        SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS!=s32Ret, EXT_RELEASE,
            "Error(%#x),SAMPLE_VPSS_GetChnFrame failed, VPSS_GRP(%d), VPSS_CHN(%d)!\n",
        ...
        //Draw rect
        stDrawRectMsg.pstRegion = (SAMPLE_SVP_NNIE_RECT_ARRAY_S *)(HI_UL)s_stRfcnRectBuf.u64PhyAddr;
        memcpy(&stDrawRectMsg.stFrameInfo, &stBaseFrmInfo, sizeof(VIDEO_FRAME_INFO_S));
        memcpy((HI_U8*)(HI_UL)s_stRfcnRectBuf.u64VirAddr, &(pstSwParam->stRect), sizeof(SAMPLE_SVP_NNIE_RECT_ARRAY_S));
        s32Ret = SAMPLE_NNIE_MsgProcess(HI_TRUE, HI_FALSE, &stDrawRectMsg);
        SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS!=s32Ret, BASE_RELEASE,
        "Error(%#x),SAMPLE_NNIE_MsgProcess failed!\n", s32Ret);

        BASE_RELEASE:
            s32Ret = SAMPLE_VPSS_ReleaseChnFrame(s32VpssGrp,as32VpssChn[0], &stBaseFrmInfo);
            if (HI_SUCCESS != s32Ret)
            {
                SAMPLE_PRT("Error(%#x),SAMPLE_VPSS_ReleaseChnFrame failed,Grp(%d) chn(%d)!\n",
                    s32Ret,s32VpssGrp,as32VpssChn[0]);
            }

        EXT_RELEASE:
            s32Ret = SAMPLE_VPSS_ReleaseChnFrame(s32VpssGrp,as32VpssChn[1], &stExtFrmInfo);
            if (HI_SUCCESS != s32Ret)
            {
                SAMPLE_PRT("Error(%#x),SAMPLE_VPSS_ReleaseChnFrame failed,Grp(%d) chn(%d)!\n",
                    s32Ret,s32VpssGrp,as32VpssChn[1]);
...
void SAMPLE_SVP_NNIE_Rfcn(void)
{
    ...
    SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, END_RFCN_1,
    ...
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,END_RFCN_2,SAMPLE_SVP_ERR_LEVEL_ERROR,
       "Error,SAMPLE_SVP_NNIE_Rfcn_ParamInit failed!\n");

    s32Ret = SAMPLE_COMM_SVP_CreateMemInfo(&s_stRfcnRectBuf, sizeof(SAMPLE_SVP_NNIE_RECT_ARRAY_S), 0);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,END_RFCN_2,SAMPLE_SVP_ERR_LEVEL_ERROR,
       "Error,SAMPLE_COMM_SVP_CreateMemInfo failed!\n");
    ...
    SAMPLE_COMM_SVP_DestroyMemInfo(&s_stRfcnRectBuf, 0);
END_RFCN_2:
...
void SAMPLE_SVP_NNIE_Ssd(void)
{
    ...
    SAMPLE_SVP_TRACE_INFO("Ssd result,big-little,start(%llu:%llu),end(%llu:%llu)\n",
                          stStartTimeSpc.tv_sec,stStartTimeSpc.tv_nsec,
                          stEndTimeSpc.tv_sec,stEndTimeSpc.tv_nsec);
    SAMPLE_SVP_TRACE_INFO("Ssd print result,big-little,pic resolution (300*300) bgr,time difference(%llu us ~= %llu ms) us\n",diff,diff/1000);
