...
static HI_S32 SAMPLE_IVE_Gmm_Init(SAMPLE_IVE_GMM_S* pstGmm, HI_U32 u32Width, HI_U32 u32Height)
{
    ...
    memset((HI_VOID*)pstGmm->stModel.u64VirAddr, 0, pstGmm->stModel.u32Size);
...
static HI_S32 SAMPLE_IVE_GmmProc(SAMPLE_IVE_GMM_INFO_S* pstGmmInfo, VIDEO_FRAME_INFO_S* pstExtFrmInfo,HI_U32 u32BaseWidth,HI_U32 u32BaseHeight)
{
    ...
    pstBlob = (IVE_CCBLOB_S*)pstGmm->stBlob.u64VirAddr;
...
static HI_VOID* SAMPLE_IVE_ViToVo(HI_VOID* pArgs)
{
    ...
    SAMPLE_IVE_DRAW_RECT_MSG_S stDrawRectMsg;
    ...
    while (HI_FALSE == s_bStopSignal)
    {
        ...
        SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS!=s32Ret, BASE_RELEASE,
          ...
          if ((HI_TRUE == bEncode) || (HI_TRUE == bVo))
        {
            memcpy(&stDrawRectMsg.stFrameInfo, &stBaseFrmInfo, sizeof(VIDEO_FRAME_INFO_S));
            memcpy(&stDrawRectMsg.stRegion, &pstGmm->stRegion, sizeof(SAMPLE_RECT_ARRAY_S));
            s32Ret = SAMPLE_IVE_MsgProcess(bVo, bEncode, &stDrawRectMsg);
            SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS!=s32Ret, BASE_RELEASE,
            "Error(%#x),SAMPLE_IVE_MsgProcess failed!\n", s32Ret);
        }
