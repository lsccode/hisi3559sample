...
/******************************************************************************
* function : IVE message process:(1.fill rect;2.send venc;3.send vo)
******************************************************************************/
static HI_S32 SAMPLE_MEDIA_IVE_Process(HI_IPCMSG_MESSAGE_S *pstMsg)
{
    VO_LAYER VoLayer = 0;
    VO_CHN VoChn = 0;
    VENC_CHN VeChn = 0;
    ...
    SAMPLE_IVE_DRAW_RECT_MSG_S *pstDrawRectMsg;

    bVo = pstMsg->as32PrivData[0];
    bEncode = pstMsg->as32PrivData[1];
    pstDrawRectMsg = (SAMPLE_IVE_DRAW_RECT_MSG_S *)pstMsg->pBody;

    (HI_VOID)SAMPLE_COMM_VGS_FillRect(&pstDrawRectMsg->stFrameInfo, &pstDrawRectMsg->stRegion, 0x0000FF00);
...
HI_S32 SAMPLE_IVE_MSG_PROC(HI_IPCMSG_MESSAGE_S *pstMsg)
{
    ...
    {
        case SAMPLE_MEDIA_CMD_IVE_DRAW_RECT_PROC:
            s32Ret = SAMPLE_MEDIA_IVE_Process(pstMsg);
            break;
