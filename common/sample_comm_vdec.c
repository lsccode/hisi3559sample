...
HI_S32 SAMPLE_COMM_VDEC_InitVBPool(HI_U32 ChnNum, SAMPLE_VDEC_ATTR *pastSampleVdec)
{
    ...
    else if (VB_SOURCE_USER == g_enVdecVBSource)
    {
        for (i = 0; i < ChnNum; i++)
        {
            ...
            {
                g_ahPicVbPool[i] = HI_MPI_VB_CreatePool(astSampleVdecBuf[i].u32PicBufSzie, pastSampleVdec[i].u32FrameBufCnt, NULL);
            ...
            {
                g_ahTmvVbPool[i] = HI_MPI_VB_CreatePool(astSampleVdecBuf[i].u32TmvBufSzie, pastSampleVdec[i].stSapmleVdecVideo.u32RefFrameNum+1, NULL);
            ...
            if (bVoPause == HI_TRUE)
            {
                ...
                for (VoLayer=0; VoLayer<VO_MAX_LAYER_NUM; VoLayer++)
                {
                    for (VoChn=0; VoChn<VO_MAX_CHN_NUM; VoChn++)
                    {
                        ...
                        if(HI_SUCCESS != s32Ret)
                        {
                            printf("HI_MPI_VO_ResumeChn(%d, %d) fail for 0x%x!\n", VoLayer, VoChn, s32Ret);
                        }
                    }
                }
                printf("VO Resume!!!");
            ...
            if(bVoPause == HI_FALSE)
            {
                ...
                for (VoLayer=0; VoLayer<VO_MAX_LAYER_NUM; VoLayer++)
                {
                    for (VoChn=0; VoChn<VO_MAX_CHN_NUM; VoChn++)
                    {
                        ...
                        if(HI_SUCCESS != s32Ret)
                        {
                            printf("HI_MPI_VO_PauseChn(%d, %d) fail for 0x%x!\n", VoLayer, VoChn, s32Ret);
                        }
                    }
                }
                printf("VO Pause...");
